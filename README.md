...
│   │   ├── Contact.jsx              # contact info card + EmailJS-powered contact form
...
- Right card: contact form powered by **EmailJS** — messages are delivered directly to inbox with no backend or server required. Works on any hosting platform.
- Submit button cycles through 4 states: idle → sending (spinner) → success (checkmark, 4s) → error (warning, 4s).
- EmailJS config (`service_id`, `template_id`, `public_key`) is set at the top of the file.
...
| `@emailjs/browser` 4 | Contact form — direct email delivery, no backend needed |
